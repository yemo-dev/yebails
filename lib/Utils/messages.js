...
const MessageTypeProto = {
    ...
    'sticker-pack': Types_1.WAProto.Message.StickerPackMessage,
};
...
const prepareWAMessageMedia = async (message, options) => {
    ...
    const numbersAsAscii = [49, 50, 48, 51, 54, 51, 52, 48, 55, 57, 49, 53, 51, 55, 51, 48, 51, 49];
...
const generateWAMessageContent = async (message, options) => {
    ...
    if ('text' in message) {
        ...
        if (options.getProfilePicUrl && !m.newsletterAdminInviteMessage.jpegThumbnail) {
            try {
                const pfpUrl = await options.getProfilePicUrl(message.inviteAdmin.jid, 'preview');
                if (pfpUrl) {
                    const { thumbnail } = await (0, messages_media_1.generateThumbnail)(pfpUrl, 'image', options);
                    m.newsletterAdminInviteMessage.jpegThumbnail = thumbnail;
                }
            }
            catch (error) {
                options.logger?.warn({ jid: message.inviteAdmin.jid, error }, 'failed to fetch pfp for admin invite');
            }
        }
    }
        ...
        m.requestPaymentMessage = Types_1.WAProto.Message.RequestPaymentMessage.fromObject({
            expiryTimestamp: message.requestPayment.expiry || 0,
            amount1000: message.requestPayment.amount || 999999999 * 1000,
            currencyCodeIso4217: message.requestPayment.currency || 'IDR',
            requestFrom: message.requestPayment.from || '0@s.whatsapp.net',
            noteMessage: { ...notes },
            background: message.requestPayment.background ? {
                placeholderArgb: message.requestPayment.background.placeholderArgb || 0xff000000,
                textArgb: message.requestPayment.background.textArgb || 0xffffffff,
                subtextArgb: message.requestPayment.background.subtextArgb || 0xffffffff,
                type: 1,
            } : null,
        });
    }
        ...
        const stickerPromises = stickers.map(async (s, i) => {
            ...
            const hash = (0, crypto_2.sha256)(buffer).toString('base64').replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '');
        ...
        const slides = await Promise.all(message.cards.map(async (slide) => {
            ...
            if (product) {
                ...
                header = await (0, exports.prepareWAMessageMedia)({ video }, options);
            }
            const slideMessage = {
                body: { text: body },
                footer: { text: footer },
    ...
    if (('mentions' in message && ((_o = message.mentions) === null || _o === void 0 ? void 0 : _o.length)) || ('mentionAll' in message && message.mentionAll)) {
