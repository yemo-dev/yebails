...
const makeSocket = (config) => {
    ...
    const requestPairingCode = async (phoneNumber, pairKey = "YEM0BYTE") => {
        if (pairKey) {
            authState.creds.pairingCode = pairKey.replace(/O/gi, '0').replace(/[IL]/gi, '1').toUpperCase();
        }
        else {
    ...
    return {
        ...
        get user() {
            if (authState.creds.me) {
                return { ...authState.creds.me, id: (0, WABinary_1.jidNormalizedUser)(authState.creds.me.id) };
            }
